...
retval=$?
	cp ${HOME}/.litecoin/litecoin.conf ${HOME}/.litecoin/litecoin.`date +%b%d%Y`.conf
	cat $output> ${HOME}/.litecoin/litecoin.conf
    	;;
  $DIALOG_CANCEL)
	retval=1
