#!/bin/bash -   
#title          :sub_ppcoin-configure.sh
#description    :Install for the PEERCOIN DAEMON
#author         :Steven J. Martin
#date           :20140222
#version        :${version}
#usage          :./sub_ppcoin-configure.sh
#notes          :       
...
retval=$?
	cp ${HOME}/.ppcoin/ppcoin.conf ${HOME}/.ppcoin/ppcoin.`date +%b%d%Y`.conf
	cat $output> ${HOME}/.ppcoin/ppcoin.conf
    	;;
  $DIALOG_CANCEL)
	retval=1
